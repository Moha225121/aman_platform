...
إذا أنشأت الخدمة يدويًا بدل استيراد App Spec، اربط قاعدة PostgreSQL واضبط `DB_CONNECTION=pgsql` و`DB_URL` بقيمة الربط الخاصة بالقاعدة. يعمل المشروع مؤقتًا بـSQLite عند غياب هذه القيم، لكن بيانات SQLite داخل الحاوية قد تُفقد عند إعادة النشر أو إعادة التشغيل.
