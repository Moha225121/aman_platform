...
إذا أنشأت الخدمة يدويًا بدل استيراد App Spec، اربط قاعدة PostgreSQL باسم `db` واضبط `DB_CONNECTION=pgsql` و`DB_URL=${db.DATABASE_PRIVATE_URL}`. يدعم المشروع أيضًا `DATABASE_URL` تلقائيًا. عند غياب قاعدة مشتركة يستخدم المشروع SQLite للبيانات وCookies مشفرة للجلسات؛ فهذا يمنع ضياع تسجيل الدخول عند انتقال الطلب بين حاويات App Platform. تبقى بيانات SQLite نفسها مؤقتة وقد تختلف بين الحاويات أو تُفقد عند إعادة النشر.
